...
namespace EducationalPortal.Infrastructure.Repositories
{
    public class CartItemsRepository : ICartItemsRepository
    {
        ...
        {
            return this._table.AnyAsync(ci => ci.Course.Id == courseId && ci.User.Email == email, cancellationToken);
        }

        public Task<List<CartItem>> GetAllAsync(string email, CancellationToken cancellationToken)
        {
        ...
        public Task<CartItem?> GetOneAsync(int id, CancellationToken cancellationToken)
        {
            return this._table.FirstOrDefaultAsync(ci => ci.Id == id, cancellationToken);
        }
        ...
        public Task<int> GetTotalPriceAsync(string email, CancellationToken cancellationToken)
        {
            return this._table.AsNoTracking()
                              .Where(ci => ci.User.Email == email)
